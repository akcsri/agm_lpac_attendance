...
A:
- **admin**: 全参加者の閲覧、CSV出力、CSVインポート
- **user1**: AGM出席管理のみ
- **user2**: AGM + LPAC出席管理

### Q: CSVインポートで一括登録できますか？

A: はい、管理者は以下が可能です：
- **ユーザーCSVインポート**: 複数のユーザーを一括作成
- **参加者CSVインポート**: 複数の参加者を一括登録/更新
- **参加者CSV出力**: 全参加者データのエクスポート

テンプレートファイル:
- `users_template.csv` - ユーザーインポート用
- `participants_template.csv` - 参加者インポート用

詳細はREADME.mdの「CSVインポート/エクスポート機能」を参照してください。
