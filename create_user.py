...
使い方:
  python create_user.py ユーザー名 パスワード --role admin
  python create_user.py ユーザー名 パスワード --role user1
  python create_user.py ユーザー名 パスワード --role user2
...
def create_user(username, password, role='user1'):
    with app.app_context():
        ...
        if created_user:
            ...
            print(f"   パスワードハッシュ長: {len(created_user.password_hash)} 文字")
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--role', type=str, default='user1', 
                       help='The role of the new user (admin, user1, or user2). Default: user1')

    args = parser.parse_args()

    # ロールの検証
    valid_roles = ['admin', 'user1', 'user2']
    if args.role not in valid_roles:
        print(f"❌ エラー: ロールは {', '.join(valid_roles)} のいずれかである必要があります")
        print(f"   指定されたロール: {args.role}")
        exit(1)
